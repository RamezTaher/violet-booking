...
const ClientsNumberDrop = ({ onChangeDirection, chosenNum }: Props) => {
  ...
  useEffect(() => {
    setSelectedNumber({
      id: chosenNum.id ? chosenNum.id : 2,
      value: chosenNum.value ? chosenNum.value : "2",
    })
  ...
  return (
    <Fragment>
      <Combobox
        className={`hidden relative justify-between lg:flex flex-col gap-2  p-2.5  w-4/12  border-b border-solid lg:border-transparent border-dark-tint`}
        as={"div"}
        ...
        {({ open }) => (
          <>
            <Combobox.Button className={"h-full flex items-center gap-2"}>
              <i className="icon-face_black_24dp text-primary text-2xl"></i>
            ...
            >
              <Combobox.Options
                className={`absolute start-0 w-full overflow-y-auto bg-white flex flex-col gap-1 p-4 z-50 ${
                  router.pathname !== "/hotel/search" ? " top-24" : "top-"
                }`}
              >
      ...
      <div className="lg:hidden   p-2.5 lg:p-5 pt-0 ">
        <div className=" flex items-center gap-2 border-b border-dark-tint border-solid">
          <i className="icon-face_black_24dp text-primary text-xl"></i>
          <div
